...
if(!submitcheck('bsubmit')){
	...
	$oA->fm_fields(array('subject','image'),0);
	$oA->fm_fields(array('keywords','abstract'),0);
	$oA->fm_fields(array('author','source','thumb','toupiao'),0);
	$oA->fm_footer();
	$oA->fm_header('活动概况');
	$oA->fm_fields(array('startdate'),0);
	$oA->fm_enddate();//到期时间
	$oA->fm_fields(array('hdnum','bmtel','yhsm'),0);
	$oA->fm_fields(array('image','content'),1); //处理排除字段后，剩余的有效字段
	$oA->fm_footer();
	
	//($title)，$title手动设置标题
	$oA->fm_header('图文信息');
	$oA->fm_fields(array('content'),0);
