...
'pre' => '',//默认的主表前缀
'where' => " a.chid='$chid'",
'from' => " {$tblprefix}archives23 a LEFT JOIN {$tblprefix}archives15 b ON a.pid3=b.aid",//sql中的FROM部分，可以通过这里JOIN其它表
'select' => "a.*,b.subject as lpname",//sql中的SELECT部分
...
//搜索项目 ****************************
//s_additem($key,$cfg)
$oL->s_additem('keyword',array('fields' => array('a.subject' => '标题','a.mname' => '会员账号','a.aid' => '文档ID','b.subject'=>'楼盘名称'),));//fields留空则默认为array('a.subject' => '标题','a.mname' => '会员','a.aid' => '文档ID')
...
if(!submitcheck('bsubmit')){
	...
	$oL->m_additem('lpname',array('title'=>'所属楼盘','type'=>'lpname','len' => 20,'pidkey' => 'pid3'));  
	foreach($oL->A['coids'] as $k){
		$oL->m_additem("ccid$k");
	}
	$oL->m_additem('valid');
	$oL->m_additem('checked',array('type'=>'bool','title'=>'审核',));
	$oL->m_additem('clicks',array('type'=>'input','title'=>'点击数','width'=>50,'view'=>'S','w' => 3,));
	...
	$oL->m_additem('awgs',array('type'=>'ucount','title'=>'报名统计','mtitle'=>'[{awgs}]','url'=>"?entry=extend&extend=dfangs&aid={aid}",'func'=>'getjhnum','cuid'=>8,'width'=>65,));
	$oL->m_additem('wanggou',array('type'=>'url','title'=>'添加','mtitle'=>'添加','url'=>"?entry=extend&extend=dfangs&aid={aid}&ht=1&admadd=1",'width'=>65,));
	$oL->m_additem('info',array('type'=>'url','title'=>'更多','mtitle'=>'查看','url'=>"?entry=extend&extend=archiveinfo&aid={aid}",'width'=>30,'view'=>'H'));
	$oL->m_additem('detail',array('type'=>'url','title'=>'编辑','mtitle'=>'详情','url'=>"?entry=extend&extend=huodongarchive&aid={aid}&arid=2",'width'=>30,));
	
	$oL->m_addgroup('{awgs}&nbsp;{wanggou}','报名');//请注意分组不能嵌套，每项只能参与一次分组
