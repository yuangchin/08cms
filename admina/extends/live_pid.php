...
in_array($chid,array(1,4,12)) || cls_message::show("请正确指定模型！");//需要限制模型范围
switch($chid){
	case 1:
...
'pid' => $pid,//合辑id
'arid' => $arid,//*** 指定合辑项目id
'orderby' => "b.inorder ASC,a.aid DESC",//合辑内指定排序,文档表合辑记录则为"a.inorderxx DESC"，xx为合辑项目id
...
if(!submitcheck('bsubmit')){
	...
	//内容列表区 **************************
	//$oL->m_header(" &nbsp;<a style=\"color:#C00\" href=\"?entry=extend&extend=loupans_load&pid=$pid\" onclick=\"return floatwin('open_arcexit',this)\">>>加载内容</a> &nbsp;",1);
    $oL->m_header(" &nbsp; <a style=\"color:#C00\" href=\"?entry=extend&extend=live_load&pid=$pid&chid=$chid\" onclick=\"return floatwin('open_arcexit',this)\">>>加载内容</a>",1);
	...
	$oL->m_additem('detail',array('type'=>'url','title'=>'编辑','mtitle'=>'详情','url'=>"?entry=extend&extend=$detailExtend&aid={aid}",'width'=>40,));
	...
	$oL->o_footer('bsubmit');
	$oL->guide_bm('&nbsp;&nbsp;&nbsp;1.合辑管理>>清除，该操作是把数据在本页面列表中删除，可以通过"加载"操作加载进来。<br/>&nbsp;<br/>','1');
