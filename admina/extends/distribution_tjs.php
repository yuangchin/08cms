...
if($cid){
	...
	if(!submitcheck('bsubmit')){
		...
		if($oA->predata['status']=='3'){
			echo "</table>\n";
			echo '<br /><input type="button" name="bsubmit" value="(成交结算)状态不能提交更新" disabled style="background:#999;"> ';
			echo "</form>\n";
		}else{
	...
	//搜索项目 **************************** 'a.subject'=>'分销活动',
	$oL->s_additem('keyword',array('fields' => array('cu.xingming' => '被推荐人','cu.dianhua' => '联系电话','cu.mname' => '推荐人帐号'),'custom'=>1));
	...
	if(!submitcheck('bsubmit')){
		...
		$oL->m_additem('mname',array('title'=>'推荐人','width'=>80));	
