...
if($cid){
	...
	//搜索项目 ****************************
	$oL->s_additem('keyword',array('fields' => array('cu.mname'=>'评论者'),'custom'=>1));
	$oL->s_additem('indays');
	...
	if(!submitcheck('bsubmit')){
		...
		//显示列表区头部 ***************
		$oL->m_header('', $aid, $aid ? " &nbsp; <a href='?entry=extend&extend=$extend='>全部评论&gt;&gt;</a>" : '');
		$oL->m_additem('selectid');	 
		$oL->m_additem('mname',array('title'=>'评论者','side'=>'L'));
        $oL->m_additem('checked',array('type'=>'bool','title'=>'审核'));
		$oL->m_additem('ip',array('type'=>'other','title'=>'来源IP'));
		$oL->m_additem('cucreate',array('type'=>'date','title'=>'回复时间'));        
	...
	//搜索项目 ****************************
	$oL->s_additem('keyword',array('fields' => array('a.subject'=>'文档标题','cu.mname' => '评论者',),'custom'=>1));
	$oL->s_additem('indays');
	$oL->s_additem('outdays');
	//搜索sql及filter字串处理
	$oL->s_deal_str(); 
	...
	if(!submitcheck('bsubmit')){
		...
		$oL->m_header( );
		$oL->m_additem('selectid'); 
		$oL->m_additem('subject',array('len'=>40,'title'=>'被评文档')); 
	
		$oL->m_additem('mname',array('title'=>'会员'));
		$oL->m_additem('checked',array('type'=>'bool','title'=>'审核'));
		$oL->m_additem('recounts',array('url'=>"?entry=extend&extend=img_reply&cid={cid}",'title'=>'回复','winsize'=>'930,480','width'=>100));
		$oL->m_additem('ip',array('type'=>'other','title'=>'来源IP'));
