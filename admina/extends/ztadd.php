...
if(!submitcheck('bsubmit')){
	...
	$oA->fm_fields(array('subject','fusubject','keywords','abstract','author','toupiao'),0);
	$oA->fm_fields(array('image','content','thumb'),1); //处理剩余的有效字段，可以传入排除字段$nos
	
	$oA->fm_footer();
	
	//($title)，$title手动设置标题
	$oA->fm_header('图文信息');
	$oA->fm_fields(array('thumb','image','content'),0);	
	$oA->fm_footer();
	
	//($title)，$title手动设置标题
	$oA->fm_header('更多设置','',array('hidden'=>1));
	...
	$oA->fm_params(array('createdate','clicks','jumpurl','customurl','relate_ids'));
	$oA->fm_param('arctpls',array('addnums'=>array(1,2,3)));
	...
	*/
	...
	$oA->sv_params(array('createdate','clicks','jumpurl','customurl','relate_ids','arctpls','enddate'));
