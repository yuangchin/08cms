...
'select' => " a.*,c.startdate ",//sql中的SELECT部分
...
if(!submitcheck('bsubmit')){
	...
	$oL->m_additem('startdate',array('type'=>'date','title'=>'开始时间',));
	$oL->m_additem('enddate',array('type'=>'date','title'=>'结束时间',));
	...
	$oL->m_additem('gllp',array('type'=>'ucount','title'=>'关联楼盘','url'=>"?entry=extend&extend=kf_pid&pid={aid}",'func'=>'gethjnum','chid'=>4,'arid'=>'32','width'=>60,));
	$oL->m_additem('ucount1',array('type'=>'ucount','title'=>'看房','url'=>"?entry=extend&extend=commu_kanfangbm&aid={aid}",'func'=>'getjhnum','cuid'=>'45','width'=>65,));
	//带参数ht，用于后台添加报名时，可以去掉验证码
	$oL->m_additem('add',array('type'=>'url','title'=>'报名','mtitle'=>'添加','url'=>"?entry=extend&extend=commu_kanfangbm&aid={aid}&admadd=1",'width'=>65,));
	$oL->m_addgroup('{ucount1}&nbsp;{add}','报名');
	$oL->m_additem('info',array('type'=>'url','title'=>'更多','mtitle'=>'查看','url'=>"?entry=extend&extend=archiveinfo&aid={aid}",'width'=>40,));
	$oL->m_additem('detail',array('type'=>'url','title'=>'编辑','mtitle'=>'详情','url'=>"?entry=extend&extend=kfarchive&aid={aid}&caid=$caid",'width'=>40,));
