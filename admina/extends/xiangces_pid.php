...
'select' => "a.*,b.subject as lpname,b.ccid1 ",//sql中的SELECT部分
...
$oL->s_additem('orderby',array(
	'options'=>array(
		0 => array('--排序方式--','a.aid DESC'),
		1 => array('辑内排序&darr;',"a.{$paixu} ASC,a.aid DESC"),            				
		2 => array('添加时间&darr;','a.createdate DESC'),        			
	)
));
...
if(!submitcheck('bsubmit')){
	...
	$hlinks .= " &nbsp; <a style=\"color:#C00\" href=\"?entry=extend&extend=xiangcearchive&pid=$pid\" onclick=\"return floatwin('open_arcexit',this)\">&gt;&gt;添加相册</a>";
	$hlinks .= " &nbsp; <a style=\"color:#C00\" href=\"?entry=extend&extend=xiangcearchives&isframe=1\" target='_blank'>&gt;&gt;所有相册</a>";
	$oL->m_header("$hlinks",1);
	$oL->m_additem('selectid');
	$oL->m_additem('caid');
	$oL->m_additem($paixu,array('type' => 'input','title'=>'排序','w' => 2));
	$oL->m_additem('subject',array('len' => 20,'url' => '#'));
	$oL->m_additem('thumb',array('type'=>'image','width'=>'100%','height'=>180));
	$oL->m_additem('detail',array('type'=>'url','title'=>'编辑','mtitle'=>'[编辑]','url'=>"?entry=extend&extend=xiangcearchive&aid={aid}",'width'=>40,'view'=>'H'));
	$oL->m_additem('littlethumb',array('type'=>'url','title'=>'设为缩略图','mtitle'=>'[设为缩略图]','url'=>"?entry=extend&extend=xiangces_pid&pid=$pid&setthumb={aid}",'width'=>40,'view'=>'H'));
	$oL->m_additem('ccid1');	
	$oL->m_additem('lpname',array('len' => 20,));	
	$oL->m_mcols_style("{thumb}<div style=\"clear:both;\"></div>{selectid}{subject}({caid})<br>排序&nbsp;{{$paixu}}&nbsp;{littlethumb}&nbsp;{detail}&nbsp;&nbsp;<br/>[{ccid1}]{lpname}");//多列文档模式定义显示项目的组合样式,默认为："{selectid} 
