...
if(!submitcheck('bsubmit')){
	...
	//分析当前会员的权限
	$oA->fm_allow();
	$arr1 = array('tgstar','tgend','hdtime','tel','qq','area','gongjiao','hdnum');
	$arr2 = array('thumb','tgimage','content'); 
	...
	$oA->fm_fields(array('keywords','abstract','dt'),0);
	$oA->fm_ccids(array());	
	$oA->fm_fields(array_merge($arr1,$arr2),1);
	$oA->fm_footer();
	...
	//展示多个属性项，管理后台默认为array(,'jumpurl','customurl','relate_ids')，会员中心默认为array('jumpurl','ucid')
	$oA->fm_params(array('clicks','createdate'));	
	...
	*/
	...
	$oA->sv_params(array('clicks','createdate'));
