...
if(!submitcheck('bsubmit')){
	...
	foreach($oL->A['coids'] as $k){
		if(in_array($k,array(19,42)) ) $oL->m_additem("ccid$k",array('view'=>'H')); 
		else $oL->m_additem("ccid$k");
	}
	$oL->m_additem('checked',array('type'=>'bool','title'=>'审核',));
	$oL->m_additem('valid');
	$oL->m_additem('clicks',array('title'=>'点击数','width'=>50,'view'=>'H','w' => 3,));
	...
	$oL->m_additem('anlinum',array('type'=>'url','title'=>'案例','mtitle'=>'[{anlinum}]','url'=>'?entry=extend&extend=designCase_pid&pid={aid}'));	
	$oL->m_additem('info',array('type'=>'url','title'=>'更多','mtitle'=>'更多','url'=>"?entry=extend&extend=archiveinfo&aid={aid}",'width'=>40,));
	$oL->m_additem('detail',array('type'=>'url','title'=>'编辑','mtitle'=>'详情','url'=>"?entry=extend&extend=design_a&aid={aid}",'width'=>40,));
