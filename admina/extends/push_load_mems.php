...
'from' => " {$tblprefix}members m LEFT JOIN {$tblprefix}members_sub s ON s.mid=m.mid   ",//sql中FROM之后的完整部分，可以通过这里JOIN其它表，例：members m LEFT JOIN members_sub s ON (s.mid=m.mid) LEFT JOIN members_$mchid c ON (c.mid=m.mid)  
);
/******************/
...
if(!submitcheck('bsubmit')){
	...
	in_array($oL->A['mchid'],array(2,3)) && $oL->m_additem('subject',array('len' => 40,'field' => 'mname','pic'=>'image'));//处理了会员标记及空间url的标题，nourl表示不需要空间url
	in_array($oL->A['mchid'],array(11,12)) && $oL->m_additem('subject',array('len' => 40,'field' => 'mname','pic'=>'pic'));//处理了会员标记及空间url的标题，nourl表示不需要空间url
	...
	$oL->A['mchid']==11 && $oL->m_additem('companynm',array('title'=>'装修公司名称','mtitle'=>'{companynm}','len' => 40));
	$oL->A['mchid']==12 && $oL->m_additem('companynm',array('title'=>'品牌商家名称','mtitle'=>'{companynm}','len' => 40));
