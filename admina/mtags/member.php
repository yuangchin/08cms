...
if(!$modeSave){
	...
	trbasic('指定来源id','mtagnew[setting][id]',empty($mtag['setting']['id']) ? '' : $mtag['setting']['id'],'text',array('guide' => '手动输入会员mid,浏览者-1(仅用于动态页或JS调用),输入为空默认为激活会员'));
	...
	if(empty($_infragment)){
		$arr = array('js' => '使用JS动态调用当前标识解析出来的内容','detail' => '需要模型字段表的内容',);
