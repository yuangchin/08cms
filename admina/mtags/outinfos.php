...
if(!$modeSave){
	...
	$str = "<select style=\"vertical-align: middle;\" name=\"mtagnew[setting][dsid]\" onchange=\"\$id('link_mtagnew_setting_dsid').innerHTML='>><a href=\'?entry=dbsources&action=viewconfigs&dsid=' + this.options[this.selectedIndex].value + '\' target=\'_blank\'>".'查看结构'."</a>';\">".
		makeoption($dsidsarr,empty($mtag['setting']['dsid']) ? 0 : $mtag['setting']['dsid']).
		"</select>&nbsp; &nbsp; <span id=\"link_mtagnew_setting_dsid\">>><a href=\"?entry=dbsources&action=viewconfigs&dsid=".(empty($mtag['setting']['dsid']) ? 0 : $mtag['setting']['dsid'])."\" target=\"_blank\">查看结构</a></span>";
	trbasic('外部数据源','',$str,'',array('guide'=>">><a href=\"?entry=dbsources&action=dbsourcesedit&isframe=1\" target=\"_blank\">设置外部数据源</a>"));
	trbasic('* 筛选查询字串','mtagnew[setting][wherestr]',empty($mtag['setting']['wherestr']) ? '' : $mtag['setting']['wherestr'],'textarea',array('guide' => '查询字串包含select、from、where、order,不要含limit。'));
	...
	if(empty($mtagnew['setting']['wherestr'])) mtag_error('请输入查询字串');
}
?>
