...
if($action == 'cnconfigs'){
	...
	if(!($cnconfig = @$cnconfigs[$cncid])) cls_message::show('请指定正确的节点组成方案');
	...
	if(!($cntpl = @$cntpls[$tid])) cls_message::show('请选择节点配置');
	...
	if(!submitcheck('bcntpldetail')){
		...
		trbasic('节点RSS模板','fmdata[rsstpl]',makeoption(array('' => '不设置') + cls_mtpl::mtplsarr('xml'),@$cntpl['rsstpl']),'select',array('guide' => cls_mtpl::mtplGuide('xml')));
		tabfooter();
		...
		for($i = 0;$i <= $cn_max_addno;$i ++){
			...
			trbasic('页面模板',"cfgsnew[$i][tpl]",makeoption(array('' => '不设置') + cls_mtpl::mtplsarr('cindex'),empty($cfgs[$i]['tpl']) ? '' : $cfgs[$i]['tpl']),'select',array('guide' => cls_mtpl::mtplGuide('cindex')));
			trbasic('静态保存格式',"cfgsnew[$i][url]",empty($cfgs[$i]['url']) ? '' : $cfgs[$i]['url'],'text',array('guide'=>'留空则按系统总设置，{$cndir}系统默认保存路径，{$page}分页页码，数字之间建议加上分隔符_或-连接。','w'=>50));
	...
	backnav('cnode','cntpls');
	if(empty($cntpls[$tid])) cls_message::show('请选择节点配置');
	...
	if(!submitcheck('bcnodescommon')){
		...
		$arr = array('-1' => '节点配置','0' => '尚未配置',);foreach($cntpls as $k => $v) $arr[$k] = $k.'-'.$v['cname'];
		...
		while($cnode = $db->fetch_array($query)){
			...
			$cnstplstr = (empty($cntpls[$cnode['tid']]['cname']) ? '-' : "$cnode[tid]-".$cntpls[$cnode['tid']]['cname']).($cnode['keeptid'] ? ' (定制)' : '');
			$lookstr = '';
		...
		$ptypearr = array();
		for($i = 0;$i <= $cn_max_addno;$i ++) $ptypearr[$i] = $i ? '附'.$i : '首页';
	...
	if(!submitcheck('bcnodedetail')){
		...
		$arr = array('0' => '不设置');foreach($cntpls as $k => $v) $arr[$k] = $k.'-'.$v['cname'];
		...
		tabheader('节点相关配置 &nbsp;- &nbsp;'.(empty($cntpls[$cnode['tid']]['cname']) ? '尚未设置' : "<a href=\"?entry=$entry&action=cntpldetail&tid=$cnode[tid]\" onclick=\"return floatwin('open_cntplsedit',this)\">>>".$cntpls[$cnode['tid']]['cname']."</a>"));
		trbasic('附加页数量','',empty($cnode['addnum']) ? '0' : $cnode['addnum'],'');
		$arr = cls_mtpl::mtplsarr('xml');
		trbasic('节点RSS模板','',empty($cnode['rsstpl']) ? '未设置' : $cnode['rsstpl'].' &nbsp;- &nbsp;'.@$arr[$cnode['rsstpl']],'');
		...
		for($i = 0;$i <= @$cnode['addnum'];$i ++){
			...
			trbasic($pvar.'静态保存格式','',empty($cnode['cfgs'][$i]['url']) ? '按系统总设置' : $cnode['cfgs'][$i]['url'],'',array('guide'=>!$i ? '{$cndir}系统默认保存路径，{$page}分页页码。': ''));
			trbasic($pvar.'是否生成静态','',empty($cnode['cfgs'][$i]['static']) ? '按系统总设置' : '保持动态','');
			trbasic($pvar.'静态更新周期','',empty($cnode['cfgs'][$i]['period']) ? '按系统总设置' : $cnode['cfgs'][$i]['period'].'(分钟)','');
