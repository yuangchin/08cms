...
if(!submitcheck('bcommudetail')) {
	...
	setChidsBar(@$commu['cfgs']['chids'],'mchid');
    setPermBar('评论权限设置', 'communew[cfgs][pmid]', @$commu['cfgs']['pmid'], 'cuadd', 'open', '');
    setPermBar('自动审核权限设置', 'communew[cfgs][autocheck]', @$commu['cfgs']['autocheck'], 'cuadd', 'check', '');
    trbasic('重复评论时间间隔(分钟)','communew[cfgs][repeattime]',@$commu['cfgs']['repeattime'],'text',array('validate' => " rule=\"int\" min=\"0\"",'w' => 10,'guide' => '单位：分钟'));
