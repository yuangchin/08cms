...
{
    //mysql的主机地址
    private static $db_host = "127.0.0.1"; //需要第三方指定ip地址

    //数据库用户名
    private static $db_user = "redfox";   //需要第三方指定自己的用户名

    //数据库密码
    private static $db_pwd = "redfox@401"; //需要第三方指定自己的库据库密码

    //数据库
    private static $db_name = "test";      //需要第三方指定数据库

    //数据库表
    private static $db_table = "tbl_session"; //需要第三方指定数据表
	...
	/**
	 * 为方便后台管理，增加构造方法用于初始化设置值
	 */
	public function __construct($db_host, $db_user, $db_pwd, $db_name, $db_table) {
 ...
 */
define("MAIN_DOMAIN", $ckdomain);   //设置主域名
 ...
 */
define("COOKIE_DOMAIN", empty($user_session) ? false : true);
 ...
 * 开启前提需要建立mysql数据表
 */
define("USER_SESSION", empty($user_session) ? false : true);
if (defined("USER_SESSION") && USER_SESSION)
{
    @ini_set("session.save_handler", "user");
    $session = new Session($dbhost, $dbuser, $dbpw, $dbname, $tblprefix . 'cross_site_session');
