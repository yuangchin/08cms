...
foreach($oL->A['coids'] as $k){
	$atitle = $k==32 ? array('title'=>'风格') : array();
...
$oL->o_additem('readd',array('limit'=>$re_refresh,'time'=>0,'fieldname'=>'refreshes'));//刷新 ,'time'=>1440
...
if(!submitcheck('bsubmit')){
	...
	$oL->m_additem('anlinum',array('type'=>'url','title'=>'案例','mtitle'=>'[{anlinum}]','url'=>"?action=dc_pid&pid={aid}",'width'=>40,));
	$oL->m_additem('info',array('type'=>'url','title'=>'更多','mtitle'=>'更多','url'=>"?action=archiveinfo&aid={aid}",'width'=>40,));
	$oL->m_additem('detail',array('type'=>'url','title'=>'编辑','mtitle'=>'详情','url'=>"?action=design_a&aid={aid}",'width'=>40,));
