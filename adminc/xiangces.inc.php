...
$arid = $lpchid==4 ? 3 : 36;//指定合辑项目id //$arid = 1;
$lpchid = in_array($lpchid,array(4,115,116)) ? $lpchid : cls_messag::show('参数错误！');
...
'chid' => $chid,//模型id，必填
'url' => "?action=$action&lpchid=$lpchid",//表单url，必填，不需要加入chid及pid
'pre' => 'a.',//默认的主表前缀
'where' => "a.mid='{$curuser->info['mid']}' AND pid$arid IN(SELECT aid FROM {$tblprefix}archives_$lpchid)",//sql中的初始化where，限定为自已的文档
'from' => "",//sql中的FROM部分
...
if(!submitcheck('bsubmit')){
	...
	//搜索区域 ******************
	backnav($lpchid==4 ? 'loupanbar' : 'loupanbus','xiangce');
	$oL->s_header();
	...
	$oL->m_additem('detail',array('type'=>'url','title'=>'编辑','mtitle'=>'[编辑]','url'=>"?action=xiangceadd&aid={aid}",'width'=>40,'view'=>'H'));
	...
	$oL->o_footer('bsubmit');
	$oL->guide_bm("本列表只显示/管理自己添加的楼盘相册；添加入口在：管理楼盘 - 具体某个楼盘内 添加。",'fix');
