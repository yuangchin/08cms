...
if(!submitcheck('bsubmit')){
	...
	//内容列表区 **************************
	$oL->m_header(" &nbsp;<a style=\"color:#C00\" href=\"?action=xiangceadd&pid=$pid\" onclick=\"return floatwin('open_arcexit',this)\">>>添加相册</a>",1);
	$oL->m_additem('selectid');
	$oL->m_additem('subject',array('len' => 20));
	$oL->m_additem('thumb',array('type'=>'image','width'=>'100%','height'=>180));
	$oL->m_additem('editself',array('title'=>'编辑','mtitle'=>'[编辑]','url'=>"?action=xiangceadd&aid={aid}",'width'=>40));
	$oL->m_additem('littlethumb',array('type'=>'url','title'=>'设为缩略图','mtitle'=>'[设为缩略图]','url'=>"?action=$action&pid=$pid&setthumb={aid}",'width'=>40));
	$oL->m_mcols_style("{thumb}<div style=\"clear:both;\"></div>{selectid}{subject} &nbsp;{editself}<br>{littlethumb}");//多列文档模式定义显示项目的组合样式,默认为："{selectid} 
