...
set_time_limit(0); global $authkey;
...
$excel = new cls_exportexcel; $extend_str = '';
if(!submitcheck('bsubmit')){
	//_header();
	echo '<link type="text/css" rel="stylesheet" href="'.$cms_abs.'images/admina/contentsAdmin.css" />';
	$excel->ShowFieldsTable($chid,$cuid,$mchid,"请选择导出数据的项目",$where_str,"?action=$action$extend_str");
	//a_guide('exportexcel');
}else{
	if(count($fmdata)<2){
		//_header(); 
	 	cls_message::show('请选择导出项目。',"?action=$action$extend_str&chid=$chid&cuid=$cuid&mchid=$mchid&aid=$aid&q=$q&p=$p");
	}
