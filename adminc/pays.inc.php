...
if(empty($pid)){
	...
	if(!submitcheck('barcsedit')){
		...
		while($item = $db->fetch_array($query)){
			...
			$stritem .= "<tr><td class=\"item\" width=\"30\"><input class=\"checkbox\" type=\"checkbox\" name=\"selectid[$pid]\" value=\"$pid\"></td>\n".
				...
				"<td class=\"item\" width=\"30\"><a href=\"?action=pays&pid=$pid\" onclick=\"return floatwin('open_pays',this)\">查看</a></td></tr>\n";
		}
		$counts = $db->result_one("SELECT count(*) FROM {$tblprefix}pays $wheresql");
		$multi = multi($counts, $mrowpp, $page, "?action=pays$filterstr");

		tabheader('支付记录列表','','',9);
		trcategory(array("<input class=\"checkbox\" type=\"checkbox\" name=\"chkall\" onclick=\"checkall(this.form, 'selectid', 'chkall')\">",array('支付方式','item2'),'支付数量','支付接口','记录日期','到帐日期','充值日期','详情'));
		echo $stritem;
		tabfooter();
		echo $multi;
		...
		cls_message::show('现金充值信息删除成功',"?action=pays&page=$page$filterstr");
	}
}else{
	...
	if(!submitcheck('bpaydetail')){
		if(!$item['transdate']){
			tabheader('修改支付信息','paydetail','?action=pays&pid='.$pid.'&forward='.rawurlencode($forward),2,1);
		}else{
			tabheader('查看支付信息');
		}
		...
		if($item['transdate']) cls_message::show('已充值支付信息不能修改');
