...
$type = 'chushou';
$coids = array(1,2,3,9,12,14,19,43);
...
if(!submitcheck('bsubmit')){	
    ...
    //房东信息显示触发JS:$oL->landlordClickJs($curuser)
        ...
        $oL->m_header($oL->landlordClickJs($curuser).((empty($mname) ? '' : $namearr[$mname]." - "))."出售房源管理");
	...
	$oL->m_addgroup('[{ccid1}]{csubject}','房源标题/小区名称<br>其它信息');//请注意分组不能嵌套，每项只能参与一次分组
        ...
        $oL->m_additem("ccid43",array());
		
		$oL->m_addgroup('{enddate}<br>{ccid43}','到期时间<br>物业类型');
	...
	$oL->m_additem('detail',array('type'=>'url','title'=>'编辑','mtitle'=>'详情','url'=>"?action=chushouadd&aid={aid}&chid=$chid&ispid4=$ispid4",'width'=>40,));
