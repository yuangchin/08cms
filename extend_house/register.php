...
defined('M_UPSEN') || define('M_UPSEN', TRUE); defined('NOROBOT') || define('NOROBOT', TRUE);
// (fxextra)扩展处理推广连接,本文找[fxextra]找所有扩展不同点;
// 尽量与原文件保持相同或相似，包含行号，一般也不另行添加行,便于与源文件对比
...
if(!submitcheck('register')){
	...
	if($mid = $newuser->useradd($mname,_08_Encryption::password($password),$email,$mchid)){
		...
		reg_exhouse($newuser,$mchid,$fxpid); //扩展; //(fxextra)扩展处理推广连接, 不另行添加行 便于与源文件对比。
		...
		if($smstelfield && $smstelval){ //手机短信认证
