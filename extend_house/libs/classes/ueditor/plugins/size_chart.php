...
class CksizeChart extends CkPublicClass {
    /**
     * 当前类对象句柄
     *
     * @var object
    ...
    public function init() {
        global $m_cookie, $db, $tblprefix, $win_id, $mcharset;
        $count = 0;
        ...
        tabheader($lpmc . ' >>> 户型图片', 'tu_list', _08_Http_Request::uri2MVC("editor=plot_pigure&action=search{$this->_uri}"));
...
CksizeChart::Setup('插入户型图片 >> (提示：直接点击图片即可插入)');