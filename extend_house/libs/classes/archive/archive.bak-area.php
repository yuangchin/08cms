...
class cls_archive extends cls_archivebase{
	...
	*/
	...
	// 区域-商圈 (2合一组合,判断参数)
	function fm_rccid1($uccid1=0){
		$fcdisabled2 = cls_env::mconfig('fcdisabled2'); 
		if($uccid1){ 
			$curuser = cls_UserMain::CurUser();
			$ccid1 = $this->arc->archive['ccid1'] = $curuser->info['szqy'];
		}
		...
		if($uccid1 && $ccid1){
			echo "\n<script>function urstCcid1(ccid1){
					var s1 = \$id('fmdata[ccid1]');
					$(s1).find('option').each(function(index, element) {
						if($(this).val()!=ccid1){
							$(this).attr('disabled',true);
						}else{
							$(this).attr('selected',true);	
						}
					});} setTimeout(\"urstCcid1('$ccid1')\",500);</script>";
		}
	}
	// 地铁-站点 (2合一组合,判断参数)
	function fm_rccid3(){
