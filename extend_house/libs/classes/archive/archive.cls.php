...
class cls_archive extends cls_archivebase{
	...
	*/
	...
	function fm_lpExist($leixing=5){
        ...
        //400分机检测
        function fm_webcallExist()
        {
            $leixing = empty($leixing) ? 0 : $leixing;
            echo '<script type="text/javascript">';
            echo 'window._08cms_validator && _08cms_validator.init("ajax","fmdata[subject]",{url:CMS_ABS+"'._08_Http_Request::uri2MVC("ajax=webcallexist&leixing=$leixing&lpname=%1").'"})';
            echo '</script>';
        }
