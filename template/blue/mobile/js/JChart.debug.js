...
;(function(_){
    function Bar(data,cfg){
        ...
        this.bindEvents = function(){
            this.on('_click',function(x,y){clickHandler(x,y,'click.bar')});
            //this.on('_doubleclick',function(x,y){clickHandler(x,y,'doubleclick.bar')});
            this.on('_longclick',function(x,y){clickHandler(x,y,'longclick.bar')});
        ...
        function clickHandler(x,y,event){
...
;(function(_){
    var Chart = function(){
        ...
        //给chart添加click longclick doubleclick事件
        this.bindTouchEvents = function(){
            var touch = {},touchTimeout,longclickDelay = 750, longclickTimeout,now, delta,offset,
            ...
            function touchstart(e){
                ...
                if (delta > 0 && delta <= 250) touch.isDoubleclick = true;
                touch.last = now;
                longclickTimeout = setTimeout(longclick, longclickDelay);
            }
            ...
            function touchend(e){
                cancelLongclick();
                if ('last' in touch){
                    //click事件，单击/双击都会触发，0延迟，建议在不使用doubleclick的环境中使用，如果要同时使用click和doubleclick，请使用singleclick
                    _this.trigger('_click',[touch.x1,touch.y1]);
                    _this.trigger('click',[touch.x1,touch.y1]);
                    if (touch.isDoubleclick) {
                        cancelAll();
                        _this.trigger('_doubleclick',[touch.x1,touch.y1]);
                        _this.trigger('doubleclick',[touch.x1,touch.y1]);
                    }else {
                        touchTimeout = setTimeout(function(){
                            touchTimeout = null;
                            _this.trigger('_singleclick',[touch.x1,touch.y1]);
                            _this.trigger('singleclick',[touch.x1,touch.y1]);
                            touch = {};
                        }, 250)
            ...
            function longclick() {
                ...
                if (touch.last) {
                    _this.trigger('_longclick',[touch.x1,touch.y1]);
                    _this.trigger('longclick',[touch.x1,touch.y1]);
                    touch = {};
                }
            }
            ...
            function cancelAll() {
                ...
                if (longclickTimeout) clearTimeout(longclickTimeout);
                touchTimeout = longclickTimeout = null;
                touch = {};
            }
        }
    }
...
;(function(_){
    function Line(data,cfg){
        ...
        function clickHandler(x,y){
            ...
            if(p){
                _this.trigger('click.point',[_this.chartData.datasets[p[3]].data[p[2]],p[2],p[3]]);
            }
        }
...
;(function(_){
    function Pie(data,cfg){
         ...
         */
        this.bindEvents = function(){
            this.on('_click',function(x,y){clickHandler(x,y,'click.pie')});
            //暂时关闭doubleclick事件
            //this.on('_doubleclick',function(x,y){clickHandler(x,y,'doubleclick.pie')});
            this.on('_longclick',function(x,y){clickHandler(x,y,'longclick.pie')});
            //添加一个默认点击事件
            this.on('click.pie',function(){return true;})
        }
  ...
  ;(function(_){
    function Polar(data,cfg){
        ...
        function clickHandler(x,y){
            ...
            if(i>-1){
                this.trigger('click.pie',[this.data[i],i]);
            }
        }
...
;(function (_) {
    function Radar(data, cfg) {
        ...
        this.draw = function (noAnim) {
            ...
            this.initScale();
            if (noAnim) {
                this.drawAllDaclickoints(1);
                this.drawScale();
            } else {
                this.doAnim(this.drawScale, this.drawAllDaclickoints);
            }
        }
        ...
        function clickHandler(x, y) {
            var p = isInPointRange(x,y);
            if(p){
                _this.trigger('click.point',[_this.data.datasets[p[3]].data[p[2]],p[2],p[3]]);
            }
        }
        ...
        this.drawAllDaclickoints = function (animPc) {
            if (animPc >= 1)pointRanges = [];
