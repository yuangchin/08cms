...
J.Router = (function($){
    ...
    var _showArticle = function(href,el){
        ...
        el&&el.addClass('active').siblings('.active').removeClass('active');
...
J.Toast = (function($){
    var toast_type = 'toast',_toast,timer,
        ...
        TEMPLATE = {
            ...
            success : '<a href="#"><i class="icon icon-f058"></i>{value}</a>',
            error : '<a href="#"><i class="icon icon-f057"></i>{value}</a></div>',
            info : '<a href="#"><i class="icon icon-f05a"></i>{value}</a>'
        }
...
J.Popup = (function($){
    var _popup,_mask,transition,clickMask2close,
        ...
        TEMPLATE = {
            alert : '<div class="popup-title">{title}</div><div class="popup-content">{content}</div><div id="popup_btn_container"><a data-target="closePopup" data-icon="icon-e603">{ok}</a></div>',
            confirm : '<div class="popup-title">{title}</div><div class="popup-content">{content}</div><div id="popup_btn_container"><a class="cancel" data-icon="close">{cancel}</a><a data-icon="icon-e603">{ok}</a></div>',
            loading : '<p><i class="icon icon-e982"></i>{title}</p>'
        };
    ...
    var show = function(options){
        ...
        }else if(settings.elId){
            _popup[0].title = settings.elId;
            html = $('#'+settings.elId).show();
        }

        //是否显示关闭按钮
        if(settings.showCloseBtn&&!_popup[0].title){
            html += '<div id="tag_close_popup" data-target="closePopup" class="icon-e606"></div>';
        }
        //popover 箭头方向
        ...
        if (_popup[0].title) {_popup.append('<div id="tag_close_popup" data-target="closePopup" class="icon-e606"></div>')};
    ...
    var hide = function(noTransition){
        ...
        if(transition && !noTransition){
            J.anim(_popup,transition[1],200,function(){
                if (_popup[0].title) {
                    $('body').append(_popup.find('#'+_popup[0].title).hide());
                }
                _popup.hide().empty();
    ...
    var confirm = function(title,content,okCall,cancelCall){
        ...
        $('#popup_btn_container [data-icon="icon-e603"]').click(function(){
            hide();
            okCall.call(this);
        });
...
/**
 * 高亮组件
 * 在zepto的click事件里注入了一个延时器，来实现点击态
 */
J.Selected = (function($){
...
//             // console.log(scroller.__proto__);
    ...
    function Refresh(selector,type,callback){
        var iscroll, scroller,refreshEl,iconEl,labelEl,topOffset,isPullDown,
            options = {
                ...
                onPullIcon : 'icon-e61c',
                onReleaseIcon  : 'icon-e620',
                onRefreshIcon : 'icon-e982',
                callback : undefined
            };
        //装载配置
        if(typeof selector === 'object'){
            ...
            if(type === 'pullUp'){
                $.extend(options,{
                    pullText: "\u4e0a\u62c9\u52a0\u8f7d\u66f4\u591a...",//上拉加载更多
                    releaseText: "\u677e\u5f00\u5f00\u7acb\u5373\u52a0\u8f7d...",//松开开立即加载...
                    refreshText: "\u52a0\u8f7d\u4e2d...",//加载中...
                    onPullIcon : 'icon-e61c'
                })
            }
        ...
        var _init = function(opts){
            ...
            var refreshTpl = '<div class="refresh-container f-asbestos"><span class="refresh-icon '+opts.onPullIcon
