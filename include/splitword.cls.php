...
class SplitWord
{
  ...
  {  	  	
  	if($loaddic)
  	{
  	  ...
  	  //高级分词，预先载入词典以提分词高速度
  	  $dicfile = dirname(__FILE__)."/encoding/spwords.csv"; 
  	  $fp = fopen($dicfile,'r');
  ...
  function SetSource($str){
	...
	if($mcharset != 'gbk'){
		if($lan_version == 'tc' && $mcharset == 'utf-8'){
			$str = cls_string::iconv($mcharset,'big5',$str);
			$str = cls_string::iconv('big5','gbk',$str);
		}else{
			$str = cls_string::iconv($mcharset,'gbk',$str);
		}
	}
  ...
  function GetIndexText($okstr,$ilen=-1){
	...
	if($mcharset != 'gbk'){
		if($lan_version == 'tc' && $mcharset == 'utf-8'){
			$okstr = cls_string::iconv('gbk','big5',$okstr);
			$okstr = cls_string::iconv('big5',$mcharset,$okstr);
		}else{
			$okstr = cls_string::iconv('gbk',$mcharset,$okstr);
		}
	}
    return trim($okstr);
  }
