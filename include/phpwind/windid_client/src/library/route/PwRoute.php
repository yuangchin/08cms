 ...
 * @version $Id: PwRoute.php 24883 2013-02-25 07:43:57Z long.shi $
 * @package library
 */
class PwRoute extends AbstractWindRoute {
	...
	protected function init($build = false, $request = null) {
		if (!$this->_init) {
			...
			$config = include CONF_PATH . 'application/phpwind.php';
			$this->default_m = isset($config['web-apps']['phpwind']['default-module']) ? $config['web-apps']['phpwind']['default-module'] : $router->getDefaultModule();
			$routeConfig = $router->getConfig('routes', 'pw', array());
			if (isset($routeConfig['default']) && $routeConfig['default']) {
