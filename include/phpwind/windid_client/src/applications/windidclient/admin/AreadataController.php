...
class AreadataController extends AdminBaseController {
	...
	public function run() {
		...
		$parentid || $parentid = 0;
		$list = $this->_loadAreaDs()->getByParentid($parentid);
		$rout = array();
		if (0 < $parentid) {
			$rout = $this->_loadAreaDs()->getAreaRout($parentid);
		}
	...
	public function updateAction() {
		...
		$this->showMessage('success', 'admin/windidclient/areadata/run?parentid=' . $parentid);
	}
	...
	public function deleteAction() {
		...
		$childs = $this->_loadAreaDs()->getByParentid($areaid);
