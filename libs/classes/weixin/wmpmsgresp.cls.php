...
class cls_wmpMsgresp extends cls_wmpBasic{
	...
	function __construct($post,$cfg=array()){
	 ...
	 function remText($text, $conv=1){
		$conv && $text = cls_w08Basic::iconv(cls_env::getBaseIncConfigs('mcharset'),'utf-8',$text);
    ...
    // 回复Image消息
    public function remImage($mediaid) {
    ...
    // 回复语音消息
    public function remVoice($mediaid) {
    ...
    // 回复视频消息
    public function remVideo($mediaid, $title="", $description="") {
    ...
    // 回复音乐消息
    public function remMusic($title = '', $description = '', $url = '', $hq_url = '') {
		$dext = "<Title><![CDATA[$title]]></Title><Description><![CDATA[$description]]></Description>";
	...
	// 回复新闻消息
	function remNews($news, $conv=1){
		$conv && $news = cls_w08Basic::iconv(cls_env::getBaseIncConfigs('mcharset'),'utf-8',$news);
	...
	function getMethod($type){
		...
		if(method_exists($this,$method)){
			...
			cls_w08Basic::debugError("getMethod:$method",$this->post,'');
		}
	}
    ...
    function savePos($type='auto'){ 
		$row = $this->_db->select()->from('#__weixin_locate')->where(array('openid'=>$this->post->FromUserName,'appid'=>$this->cfg['appid']))->exec()->fetch();
		if(!$type) return $row;
		$data = array(
			'type' => $type,
		...
		if($row){
			$this->_db->update('#__weixin_locate', $data)->where(array('openid'=>$this->post->FromUserName,'appid'=>$this->cfg['appid']))->exec();
		}else{ 
			$data = $data + array(
				'appid' => $this->cfg['appid'],
				'openid' => $this->post->FromUserName,
			);
			$this->_db->insert('#__weixin_locate', $data)->exec(); 
		}
	}
