...
class cls_wmpMsgsend extends cls_wmpBasic{
    ...
    public function sendText($openid, $content){
		...
		return cls_w08Basic::jsonDecode($data,$this->msgSend);
    }
     ...
     * @return bool|mixed
     */
    public function sendMusic($openid, $url, $hq_url, $media_id, $title = "", $description = "") {
		$url = $this->msgSend."?access_token={$this->actoken}";
		$message = array();
		$message['touser'] = $openid;
     ...
     * @throws Exception
     */
    public function sendNews($openid, $items) {
		$url = $this->msgSend."?access_token={$this->actoken}";
		$message = array();
		$message['touser'] = $openid;
