...
//class cls_w08Tester{ 
	...
	static function showInfo($info){ 	
		...
		if(strstr($info,'<xml>') && strstr($info,'</xml>')){
			$info = cls_w08Basic::iconv('utf-8',cls_w08Basic::getConfig('mcharset','baseinc'),$info);
		}
		$flags = array('ToUserName','FromUserName','CreateTime','MsgType','/xml','Content','Event','EventKey','Ticket','Latitude','Longitude','Precision'); //,'',''
		foreach($flags as $k){
			$info = str_replace("<$k>","\n<$k>",$info);
		}
	...
	static function getMessage($mtxt=''){	
		$post = self::getPost();
		$weMsg = new cls_wmpMsgresp($post,1);
	...
	static function getSignurl($wecfg=array(), $apiurl=''){
		...
		$url = $apiurl ? $apiurl : cls_env::mconfig('cms_abs')."index.php?/weixin/&iswxtest=1";
...
class basKeyid{
	...
	static function kidRand($length=8){
		$types = 0;
		return cls_string::Random($length, $types);
	}
