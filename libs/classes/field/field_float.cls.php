...
class cls_field_float extends cls_fieldconfig{
    ...
    public static function _sv_custom_region(){
		foreach(array('vdefault','min','max') as $key){
			...
			if(self::$newField[$key] != '') self::$newField[$key] = floatval(self::$newField[$key]);
		}
	}
	# 表单之默认输入值
    protected static function _fm_vdefault(){
		$Value = self::$isNew ? '' : self::$oldField['vdefault'];
		trbasic('默认输入值','fmdata[vdefault]',$Value,'text',array('validate'=>makesubmitstr('fmdata[vdefault]',0,'number',0,10)));
	}	
	# 表单之输入值范围限制
    protected static function _fm_min_max(){
		...
		trrange('输入值范围限制', array('fmdata[min]',$ValueMin,'','&nbsp; -&nbsp; ',5, 'validate' => makesubmitstr('fmdata[min]',0,'number',0,10)),
								array('fmdata[max]',$ValueMax,'','',5, 'validate' => makesubmitstr('fmdata[max]',0,'number',0,10)));
	}
