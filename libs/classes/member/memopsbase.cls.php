<?php
/**
 *  memopsbase.cls.php  会员列表管理中的批量操作基类	 
 *
 *
...
class cls_memopsbase{
	...
	public $auser	= NULL;//批量操作中的指定会员，只在数据保存时使用
	...
	*/
	...
	protected function type_ugid($key,$mode = 0){
		...
		if(!$gtid || ($gtid== 2) || empty($grouptypes[$gtid]) || ($grouptypes[$gtid]['mode'] > 1)) return $this->del_item($key);//排除非手动项及管理组系
		if(!($ugidsarr = ugidsarr($gtid,$this->A['mchid'],1))) return $this->del_item($key);
		$cfg = &$this->cfgs[$key];
		if(!$mode){//初始化
		}elseif($mode == 1){//显示
			if(empty($cfg['title'])) $cfg['title'] = $grouptypes[$gtid]['cname'];
			trbasic($this->input_checkbox($key,$cfg['title']),"{$this->A['opre']}$key",makeoption(array(0 => '退出会员组') + $ugidsarr),'select');
		}elseif($mode == 2){//数据
	...
	protected function user_static($mode = 0){
		...
		if(!$mode){//初始化
			...
			if(empty($cfg['title'])) $cfg['title'] = '生成静态';
			return $this->input_checkbox($key,$cfg['title'],1);
		}elseif($mode == 2){//数据
			global $mspacepmid;
	...
	protected function user_unstatic($mode = 0){
		...
		if(!$mode){//初始化
			...
			if(empty($cfg['title'])) $cfg['title'] = '取消静态';
			return $this->input_checkbox($key,$cfg['title'],1);
		}elseif($mode == 2){//数据
